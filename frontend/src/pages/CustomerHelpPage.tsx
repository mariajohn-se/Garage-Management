import { useState } from 'react';
import { customerApi, Customer } from '../api/partyApi';
import { ApiError } from '../api/client';

export function CustomerHelpPage() {
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<Customer[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [searched, setSearched] = useState(false);

  async function handleSearch() {
    if (!query.trim()) return;
    setLoading(true);
    setError(null);
    setSearched(true);
    try {
      setResults(await customerApi.help(query.trim()));
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Unable to search customers.');
    } finally {
      setLoading(false);
    }
  }

  return (
    <div className="section-card" data-testid="custhelp-panel">
      <h2>Customer Lookup</h2>
      <div className="filter-bar">
        <input
          data-testid="custhelp-search"
          placeholder="Search by name..."
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
        />
        <button className="btn-primary" data-testid="custhelp-submit" onClick={handleSearch} disabled={!query.trim()}>
          Search
        </button>
      </div>

      {error && <div className="error-state">{error}</div>}
      {loading && <div className="empty-state">Searching...</div>}
      {!loading && searched && !error && results.length === 0 && (
        <div className="empty-state">No matching customers found.</div>
      )}
      {!loading && results.length > 0 && (
        <table className="data-table">
          <thead>
            <tr>
              <th>Name</th>
              <th>Phone</th>
              <th>Area</th>
            </tr>
          </thead>
          <tbody>
            {results.map((c) => (
              <tr key={c.custId} data-testid={`custhelp-row-${c.custId}`}>
                <td>{c.name}</td>
                <td>{c.phone1 || c.phone2 || '—'}</td>
                <td>{c.area ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      )}
    </div>
  );
}
